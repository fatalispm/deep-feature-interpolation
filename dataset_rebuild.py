...
def rebuild_dataset(interactive=True):
    ...
    fields = ['5_o_Clock_Shadow', 'Arched_Eyebrows', 'Attractive', 'Bags_Under_Eyes', 'Bald', 'Bangs', 'Big_Lips',
              ...
              'Wearing_Lipstick', 'Wearing_Necklace', 'Wearing_Necktie', 'Young', 'asian', 'baby', 'black',
              'brown_eyes', 'child', 'color_photo', 'eyes_open', 'flash', 'flushed_face', 'frowning',
              'fully_visible_forehead', 'harsh_lighting', 'indian', 'middle_aged', 'mouth_closed', 'mouth_wide_open',
              'no_eyewear', 'obstructed_forehead', 'outdoor', 'partially_visible_forehead', 'posed_photo', 'round_face',
              'round_jaw', 'senior', 'shiny_skin', 'soft_lighting', 'square_face', 'strong_nose_mouth_lines',
              'sunglasses', 'teeth_not_visible', 'white']
    ...
    if os.path.exists('datasets/facemodel/history.npz'):
        dedup_descriptors = npz.NpzLog('datasets/facemodel/history.npz', shape=(0, 1024), dtype=numpy.uint8,
                                       buflen=len(T))
    else:
    ...
    with open('datasets/facemodel/attributes.npz', 'wb') as f:
        numpy.savez(f, scores=new_scores, landmarks=new_landmarks, filelist=new_filelist)
    dedup_descriptors.write('datasets/facemodel/history.npz')
    if interactive:
        dellist = list(dellist)
        if len(dellist) > 0:
            print(' '.join(dellist))
            if input('Type "yes" to automatically remove {} images: '.format(len(dellist))) == 'yes':
                for x in dellist:
        ...
        if len(failed) > 0:
            print(' '.join(failed))
            if input('Type "yes" to automatically remove {} images: '.format(len(failed))) == 'yes':
                for x in failed:
                    if os.path.exists(x): os.unlink(x)
                print('Images deleted, need to rebuild dataset again.')
